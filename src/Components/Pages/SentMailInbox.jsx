    ...
    const getAllMails = async () => {
      try {
        ...
        let response = await axios.get(
          "http://localhost:3000/email/getSentMail",
  ...
  return Mails && Mails.length > 0 ? (
    Mails.map((item) => {
      return (
        <div
          key={item._id}
          className={` rounded-2xl md:w-[80%] mx-auto p-2 flex justify-between  border  my-2  cursor-pointer bg-gray-300`}>
          <div
            className="flex tuncate overflow-hidden"
            onClick={(e) => {
              e.preventDefault();
