...
const SingleSentEmail = () => {
    ...
    const getSingleMail = async () => {
      try {
        ...
        let response = await axios.get(
          `http://localhost:3000/email/get/${url.id}`,
  ...
  return (
    <div className=" w-[80%] mx-auto md:w-[60%] my-3 min-h-[700px] flex flex-col  border p-3">
      <div className=" md:text-2xl my-2 md:my-3 ">
        <div className="p-2">
          <span className="border-4 rounded-lg p-1 ">Sent To : </span>
        </div>
        <div className="p-2">
          <span className="border-4 p-1 border-gray-400 rounded-xl ">
            {singleMail.receiver}
          </span>
        </div>
      </div>
